...
simResult <-
  lapply(simDat,
         function(dat) {
           fit_bent <- balance(Z = dat$z, X = model.matrix( ~ x1 + x2 + x3 + x4, data = dat), estimand = "ATE")
           est_bent <- estimate(fit_bent, Y = dat$y)
           matrix(c(est_bent$estimate,
                    est_bent$variance,
                    as.integer(est_bent$estimate - sqrt(est_bent$variance) * 1.96 <= dat$tau &
                               est_bent$estimate + sqrt(est_bent$variance) * 1.96 >= dat$tau)),
                  nrow = 1,
                  dimnames = list(character(0), c("tau_bent", "var_bent", "cp_bent")))
         })

simResult <- do.call(rbind, simResult)

testthat::expect_equal(colMeans(simResult),
                       c(tau_bent = 19.9701735973048,
                         var_bent = 0.427706347693399,
                         cp_bent = 0.945))
...
testthat::expect_equal(colMeans(simResult),
                       c(tau_bent = 20.0035991104368,
                         var_bent = 0.0253551454642127,
                         cp_bent = 0.951))
